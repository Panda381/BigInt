...
// crc.cpp - CRC32 check sum code

#include "../include.h"

// CRC-32 table (1 KB)
const u32 Crc32Tab[256] = {
...
u32 Crc32_1(u32 crc, u8 val)
{
	crc = ~crc;
	crc = Crc32Tab[(crc ^ val) & 0xff] ^ (crc >> 8);
	return ~crc;
}
...
{
	return Crc32_Buf(CRC32_INIT, buf, len);
}
...
#define CrcPattern3 Crc32Tab

Bool CrcCheck()
{
	// Check CRC-32 table
	if (!Crc32Check()) return False;

	// Check CRC-32 pattern
	u32 crc = CRC32_INIT;
	int i = CRCPATN1;
	const u8* s = (const u8*)CrcPattern1;
	for (; i > 0; i--) crc = Crc32_1(crc, *s++);
	if (crc != 0xCBF43926) return False;

	crc = CRC32_INIT;
	i = CRCPATN2;
	s = (const u8*)CrcPattern2;
	for (; i > 0; i--) crc = Crc32_1(crc, *s++);
	if (crc != 0xA8E10F6D) return False;

	crc = CRC32_INIT;
	i = CRCPATN3;
	s = (const u8*)CrcPattern3;
	for (; i > 0; i--) crc = Crc32_1(crc, *s++);
	if (crc != 0x6FCF9E13) return False;

	if (Crc32(CrcPattern1, CRCPATN1) != 0xCBF43926) return False;
	if (Crc32(CrcPattern2, CRCPATN2) != 0xA8E10F6D) return False;
	if (Crc32(CrcPattern3, CRCPATN3) != 0x6FCF9E13) return False;
